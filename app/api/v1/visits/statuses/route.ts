...
import type { ListVisitStatusesResponse, SaveVisitStatusResponse } from './types';

import { createVisitStatusCommand } from '@/app/api/lib/modules/visit-status/commands/create-visit-status-command';
import { getVisitStatusesQuery } from '@/app/api/lib/modules/visit-status/queries/get-visit-statuses-query';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const queryResult = await getVisitStatusesQuery({
    ...
    return NextResponse.json<ListVisitStatusesResponse>({
      data,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await createVisitStatusCommand(payload, tenantSession.tenantId);
    ...
    return NextResponse.json<SaveVisitStatusResponse>(
      { data: result.data },
