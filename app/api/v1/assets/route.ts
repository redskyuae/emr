...
import type { ListAssetsResponse, SaveAssetResponse } from './types';

import { createAssetCommand } from '@/app/api/lib/modules/asset/commands/create-asset-command';
import { getAssetsQuery } from '@/app/api/lib/modules/asset/queries/get-assets-query';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const query =
      ...
      undefined;
    const categoryId = parseOptionalPositiveInteger(request.nextUrl.searchParams.get('categoryId'));
    const statusId = parseOptionalPositiveInteger(request.nextUrl.searchParams.get('statusId'));

    const safePage = Math.max(1, Math.floor(page));
    const safeLimit = Math.min(999, Math.max(1, Math.floor(limit)));

    const queryResult = await getAssetsQuery({
    ...
    return NextResponse.json<ListAssetsResponse>({
      data,
      meta: {
        total,
        totalPages,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await createAssetCommand(payload, tenantSession.tenantId);
