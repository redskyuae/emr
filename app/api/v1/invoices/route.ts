...
import type { ListInvoicesResponse, SaveInvoiceResponse } from './types';

import { createInvoiceCommand } from '@/app/api/lib/modules/invoice/commands/create-invoice-command';
import { getInvoicesQuery } from '@/app/api/lib/modules/invoice/queries/get-invoices-query';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const queryResult = await getInvoicesQuery({
      tenantId: tenantSession.tenantId,
      page: safePage,
      limit: safeLimit,
      query,
      status,
      patientId: patientId && Number.isInteger(patientId) && patientId > 0 ? patientId : undefined,
    });
    ...
    return NextResponse.json<ListInvoicesResponse>({
      data,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await createInvoiceCommand(payload, tenantSession.tenantId);
    ...
    return NextResponse.json<SaveInvoiceResponse>(
      { data: result.data },
