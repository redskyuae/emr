...
import type { CancelAppointmentResponse } from './types';
...
export async function POST(request: NextRequest, context: AppointmentRouteContext) {
  try {
    ...
    const result = await cancelAppointmentCommand(id, payload, tenantSession.tenantId);
    ...
    return NextResponse.json<CancelAppointmentResponse>({ data: result.data });
  } catch {
    return NextResponse.json(
