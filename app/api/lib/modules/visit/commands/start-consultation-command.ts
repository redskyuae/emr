...
export async function startConsultationCommand(
  id: unknown,
  tenantId: unknown
  ...
  const result = await visitRepository.startConsultation(
  ...
  if (result.outcome === 'appointment-status-not-configured') {
    return {
      success: false,
      errors: ['Appointment status is not configured.'],
  ...
  if (result.outcome === 'invalid-status') {
    return {
      success: false,
      errors: [`Visit ${result.data.visitNumber} cannot be started from its current status.`],
      status: StatusCodes.CONFLICT,
    };
  }
