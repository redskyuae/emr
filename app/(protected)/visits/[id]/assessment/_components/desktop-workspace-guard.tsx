...
export function DesktopWorkspaceGuard({ children }: { children: ReactNode }) {
  return (
    <>
      <Card className="shadow-fluent-2 [@media(min-width:1400px)]:hidden">
        <CardContent className="flex min-h-80 flex-col items-center justify-center gap-3 p-8 text-center">
          ...
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground max-w-lg text-sm">
              This safety-focused cockpit needs a viewport of at least 1400 pixels wide. Shorter
              desktop screens can scroll within each workflow column.
            </p>
          </div>
        </CardContent>
      </Card>

      <div className="hidden [@media(min-width:1400px)]:block">{children}</div>
    </>
  );
}
