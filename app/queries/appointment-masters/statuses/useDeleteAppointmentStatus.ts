'use client';

import { useMutation, useQueryClient, type UseMutationOptions } from '@tanstack/react-query';

import { parseApiError } from '@/app/queries/api-error';
import { APPOINTMENT_STATUSES_KEY } from './useAppointmentStatuses';

async function deleteAppointmentStatus(id: number): Promise<void> {
  const response = await fetch(`/api/v1/appointments/statuses/${id}`, {
    method: 'DELETE',
    credentials: 'same-origin',
  });

  if (!response.ok) {
    throw await parseApiError(response, 'Could not delete Appointment Status');
  }
}

type UseDeleteAppointmentStatusOptions = Omit<
  UseMutationOptions<void, Error, number>,
  'mutationFn'
>;

export function useDeleteAppointmentStatus(options?: UseDeleteAppointmentStatusOptions) {
  const queryClient = useQueryClient();
  const { onSuccess, ...rest } = options ?? {};

  return useMutation({
    ...rest,
    mutationFn: deleteAppointmentStatus,
    onSuccess: async (...args) => {
      await queryClient.invalidateQueries({ queryKey: APPOINTMENT_STATUSES_KEY });
      await onSuccess?.(...args);
    },
  });
}
